              ...
              {services.map((item) => (
                <div key={item.serviceId} className="flex items-center gap-3 p-3 bg-orange-50 border border-orange-100 rounded-xl">
                  <div className="w-9 h-9 rounded-lg bg-white overflow-hidden shrink-0 flex items-center justify-center">
                    {item.service?.imageUrl ? <img src={item.service.imageUrl} alt={item.service.title} className="w-full h-full object-cover" /> : <PackageIcon size={14} className="text-orange-300" />}
                  </div>
...
export default function PackagesPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return [...packages]
      .filter((pkg) => !q || pkg.title.toLowerCase().includes(q) || pkg.id.toLowerCase().includes(q) || (pkg.description || "").toLowerCase().includes(q))
      .sort((a, b) => {
        let cmp = 0;
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setError("Failed to delete package. Restart API if this endpoint was just added.");
  ...
  const handleToggle = async () => {
    ...
    try {
      ...
      setPackages((current) => current.map((pkg) => (pkg.id === toggleTarget.id ? { ...pkg, status: newStatus } : pkg)));
      setSuccess(`"${toggleTarget.title}" ${newStatus === "ACTIVE" ? "activated" : "deactivated"}`);
      setToggleTarget(null);
      window.setTimeout(() => setSuccess(""), 3000);
    } catch {
      setError("Failed to update package status. Restart API if this endpoint was just added.");
      window.setTimeout(() => setError(null), 4000);
    }
  };
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between gap-4">
        ...
        <Link href="/vendor/packages/add" className="flex items-center gap-2 bg-orange-500 hover:bg-orange-600 text-white font-semibold px-4 py-2.5 rounded-xl text-sm transition-colors shadow-sm">
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-white rounded-xl border border-gray-100 px-4 py-3 text-center"><p className="text-2xl font-bold text-gray-900">{packages.length}</p><p className="text-xs text-gray-500 mt-0.5">Total</p></div>
        <div className="bg-white rounded-xl border border-gray-100 px-4 py-3 text-center"><p className="text-2xl font-bold text-green-600">{packages.filter((pkg) => pkg.status === "ACTIVE").length}</p><p className="text-xs text-gray-500 mt-0.5">Active</p></div>
        <div className="bg-white rounded-xl border border-gray-100 px-4 py-3 text-center"><p className="text-2xl font-bold text-gray-500">{packages.filter((pkg) => pkg.status !== "ACTIVE").length}</p><p className="text-xs text-gray-500 mt-0.5">Inactive</p></div>
      </div>

      <div className="relative">
        <Search size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-400" />
        <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search packages by title, description, or ID..." className="w-full pl-10 pr-4 py-2.5 text-sm border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-orange-200 focus:border-orange-400 bg-white" />
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
                    ...
                    <tr key={pkg.id} className="hover:bg-orange-50/30 transition-colors">
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 rounded-lg bg-orange-50 flex items-center justify-center shrink-0"><PackageIcon size={16} className="text-orange-500" /></div>
                          <div className="min-w-0">
                            ...
                            {pkg.description && <p className="text-xs text-gray-500 mt-1 max-w-[340px] truncate">{pkg.description}</p>}
                          </div>
                        </div>
                      </td>
                      <td className="px-4 py-3">
                        <div className="flex flex-wrap gap-1 max-w-[240px]">
                          ...
                          {services.length > 2 && <span className="text-xs bg-orange-100 text-orange-600 px-2 py-0.5 rounded-full">+{services.length - 2} more</span>}
                          {services.length === 0 && <span className="text-xs text-gray-400">No services</span>}
                        </div>
                      </td>
                      <td className="px-4 py-3">
                        <div className="text-sm font-semibold text-gray-800 whitespace-nowrap">{packageAmount(pkg).toLocaleString()} {packageCurrency(pkg)}</div>
                        <div className="text-xs text-gray-400">{pkg.price_unit || pkg.priceUnit || "package"}</div>
                      </td>
                      <td className="px-4 py-3">
                        <button onClick={() => setToggleTarget(pkg)} className={`inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1 rounded-full transition-all ${pkg.status === "ACTIVE" ? "bg-green-100 text-green-700 hover:bg-green-200" : "bg-gray-100 text-gray-500 hover:bg-gray-200"}`}>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-center gap-1">
                          <button onClick={() => setViewTarget(pkg)} title="View Details" className="p-1.5 rounded-lg text-gray-400 hover:text-blue-500 hover:bg-blue-50"><Eye size={15} /></button>
                          <Link href={`/vendor/packages/edit/${pkg.id}`} title="Edit" className="p-1.5 rounded-lg text-gray-400 hover:text-orange-500 hover:bg-orange-50"><Edit3 size={15} /></Link>
                          <button onClick={() => setDeleteTarget(pkg)} title="Delete" className="p-1.5 rounded-lg text-gray-400 hover:text-red-500 hover:bg-red-50"><Trash2 size={15} /></button>
                        </div>
                      </td>
                    </tr>
                  );
        ...
        {filtered.length > 0 && <div className="px-4 py-3 border-t border-gray-50 bg-gray-50/50 text-xs text-gray-400 text-right">Showing {filtered.length} of {packages.length} packages</div>}
      </div>

      <PackageDetailModal pkg={viewTarget} open={!!viewTarget} onClose={() => setViewTarget(null)} />
      <ConfirmModal open={!!deleteTarget} title="Delete Package?" message={`"${deleteTarget?.title}" will be permanently deleted.`} confirmLabel="Delete" confirmClass="bg-red-500 hover:bg-red-600" icon={<Trash2 size={24} className="text-red-500" />} onConfirm={handleDelete} onCancel={() => setDeleteTarget(null)} />
      <ConfirmModal open={!!toggleTarget} title={toggleTarget?.status === "ACTIVE" ? "Deactivate Package?" : "Activate Package?"} message={toggleTarget?.status === "ACTIVE" ? `"${toggleTarget?.title}" will be hidden from customers.` : `"${toggleTarget?.title}" will become visible to customers.`} confirmLabel={toggleTarget?.status === "ACTIVE" ? "Deactivate" : "Activate"} confirmClass={toggleTarget?.status === "ACTIVE" ? "bg-gray-600 hover:bg-gray-700" : "bg-green-500 hover:bg-green-600"} icon={toggleTarget?.status === "ACTIVE" ? <ToggleLeft size={24} className="text-gray-500" /> : <ToggleRight size={24} className="text-green-500" />} onConfirm={handleToggle} onCancel={() => setToggleTarget(null)} />
    </div>
  );
}
