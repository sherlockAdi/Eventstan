...
import { Eye, RefreshCw, Package, Check, X, Star } from "lucide-react";
...
export default function PromotionPackagesPage() {
  ...
  const [pendingPromotionToggle, setPendingPromotionToggle] = useState<MarketingPackage | null>(null);
  const [promotedPackages, setPromotedPackages] = useState<Set<string>>(new Set());
  const ITEMS_PER_PAGE = 10;

  useEffect(() => {
    fetchData();
    loadPromotedPackages();
  }, []);

  const loadPromotedPackages = () => {
    const saved = localStorage.getItem("promotedMarketingPackages");
    if (saved) {
      setPromotedPackages(new Set(JSON.parse(saved)));
    }
  };

  const savePromotedPackages = (promotedSet: Set<string>) => {
    localStorage.setItem("promotedMarketingPackages", JSON.stringify(Array.from(promotedSet)));
  };

  const handlePromotionToggleClick = (pkg: MarketingPackage) => {
    setPendingPromotionToggle(pkg);
    setIsPromotionConfirmOpen(true);
  };

  const confirmPromotionToggle = () => {
    if (pendingPromotionToggle) {
      const packageId = pendingPromotionToggle.id;
      const packageName = pendingPromotionToggle.name;
      const isCurrentlyPromoted = promotedPackages.has(packageId);
      
      const newSet = new Set(promotedPackages);
      ...
      setPromotedPackages(newSet);
      savePromotedPackages(newSet);
      
      if (isCurrentlyPromoted) {
        toast.success(`${packageName} removed from promotions`);
      } else {
        toast.success(`${packageName} added to promotions`);
      }
      
      setPendingPromotionToggle(null);
  ...
  const packagesWithPromotion = filteredPackages.map(pkg => ({
    ...pkg,
    isPromoted: promotedPackages.has(pkg.id)
  }));

  const sortedPackages = [...packagesWithPromotion].sort((a, b) => {
  ...
  const paginatedData = sortedPackages.slice(
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            ...
            <span className="text-orange-600 ml-1">
              • {promotedPackages.size} promoted
            </span>
          </p>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50">
                ...
                <th className="text-left text-xs font-semibold text-gray-500 uppercase tracking-wider px-6 py-3">Promotion</th>
                <th className="text-left text-xs font-semibold text-gray-500 uppercase tracking-wider px-6 py-3">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-50">
              {paginatedData.map((pkg) => (
                <tr key={pkg.id} className={`hover:bg-gray-50/50 transition-colors ${pkg.isPromoted ? 'bg-orange-50/30' : ''}`}>
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        {pkg.isPromoted && (
                          <span className="ml-2 text-[10px] bg-orange-100 text-orange-600 px-2 py-0.5 rounded-full font-medium">Promoted</span>
                        )}
                        {pkg.description && (
                    ...
                    <div className="flex items-center gap-2">
                      <label className="relative inline-flex items-center cursor-pointer">
                        <input
                          ...
                          checked={pkg.isPromoted}
                          onChange={() => handlePromotionToggleClick(pkg)}
                        />
            ...
            <div className="relative bg-white rounded-2xl shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
              <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${selectedPackage.popular ? 'bg-orange-50' : 'bg-gray-50'}`}>
                    {promotedPackages.has(selectedPackage.id) ? (
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      {promotedPackages.has(selectedPackage.id) && (
                        <span className="text-xs bg-orange-100 text-orange-600 px-2 py-1 rounded-full font-medium">Promoted</span>
                      )}
                    </div>
              ...
              <div className="px-6 py-6 space-y-6">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <label className="relative inline-flex items-center cursor-pointer">
                        <input
                          type="checkbox"
                          className="sr-only peer"
                          checked={promotedPackages.has(selectedPackage.id)}
                          onChange={() => handlePromotionToggleClick(selectedPackage)}
                        />
                        <div className="w-9 h-5 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-orange-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-orange-500"></div>
                      </label>
                    </div>
                    <p className="text-sm font-medium text-gray-900 mt-2">
                      {promotedPackages.has(selectedPackage.id) ? "Promoted" : "Not Promoted"}
                    </p>
                    <p className="text-xs text-gray-500">Show in promotions tab</p>
                  </div>
                </div>
                ...
                <div className="pt-4 border-t border-gray-100">
                  ...
                  <p className="text-xs text-gray-400 mt-1">Vendor ID: {selectedPackage.vendorId}</p>
                </div>
              </div>
              
              <div className="sticky bottom-0 bg-white border-t border-gray-200 px-6 py-4 flex justify-end gap-3">
                <button
                  onClick={() => handlePromotionToggleClick(selectedPackage)}
                ...
                >
                  <Star size={16} />
                  {promotedPackages.has(selectedPackage.id) ? 'Remove from Promotions' : 'Add to Promotions'}
                </button>
                <button
                  onClick={closeModal}
      ...
      <ConfirmModal
        ...
        title={pendingPromotionToggle && promotedPackages.has(pendingPromotionToggle.id) ? "Remove from Promotions" : "Add to Promotions"}
        message={
          pendingPromotionToggle && promotedPackages.has(pendingPromotionToggle.id)
            ? `Are you sure you want to remove "${pendingPromotionToggle?.name}" from promotions?`
            : `Are you sure you want to add "${pendingPromotionToggle?.name}" to promotions?`
        }
      />
    </div>
  );
